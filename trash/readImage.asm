...
read_image:
	...
	lb    $s7, 1($s1)            	# load 2nd character of header - magic number
...
continue:
	...
	# check if the file is type P5 or P2
	#beq  $s7, '5', write_P5
...
write_P2:
	...
	move $t0, $v0			# save the address of the allocated memory to $t0
	...
	#addi $t1, $t0, 12		# set $t1 point to the array
	#addi $t2, $zero, 0		# set the counter
	
	# read P2 character by character
	#la   $a0, 12($t0)		# set char pointer to the first element in buffer
	#jal  atoi			# convert ascii to int
	#la   $t3, 0($a0)		# save char pointer in $t3
	#lb   $t4, 0($v0)
	#sb   $t4, 0($t1)		# save the read number into struct
	#addi $t2, $t2, 1		# increase element counter
	#addi $t1, $t1, 1		# increase the pointer
	#addi $t3, $t3, 1		# move the pointer to next element in buffer
	
write_content:	
	#beq  $t2, $s6, end_loop		# if all the elements are written, end writing
	
	# if currently point to white space or \n, skip
	#lb    $t4, 0($t3)            	# transfers one byte of data from buffer to a register.

  	#blt   $t0, 0x30, next_loop    	# if the ASCII code is less than 48 (ASCII of zero is 48) 
  	#bgt   $t0, 0x39, next_loop    	# if the ASCII code is greater than 57 (ASCII of nine is 57)
  					# skip
  					
  	# else convert ascii to int and store in struct
  	#la    $a0, 0($t3)
  	#jal   atoi
  	#la    $t3, 0($a0)
  	#sb    $v0, 0($t1)
...
next_loop:
	#addi $t3, $t3, 1		# check next char in buffer
